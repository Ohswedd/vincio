...
| 25 | [`statistical_certificates`](25_statistical_certificates.py) | The **statistical** claims a data answer makes, certified on the verified-reasoning surface. `TrendVerifier` recomputes an OLS slope and R² from the **cited cells** of a query result; `CorrelationVerifier` recomputes a correlation and **refuses a correlation stated as causation** with no controls — and **refutes** a controlled claim whose association vanishes once the confounder is partialled out (the partial correlation collapses), while a genuine driver that survives the control is verified; `IntervalVerifier` recomputes a stated confidence interval; `ForecastVerifier` re-runs a declared deterministic model and **repairs** a wrong projection through the bounded self-correction loop. Every statistic is bound to its cited cells (a value swapped after citation is refused) and the certificate re-derives its verdict from the bytes. The analytical-claim analogue of the arithmetic kernels (example 09). |
