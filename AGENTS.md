...
vincio/tasks          the ergonomic front door: one-line, task-shaped constructors rag / extractor / tool_agent / evaluation / chat over ContextApp plus the fluent, immutable Flow; each @experimental, re-exported at top level, and lowers byte-identical to ContextApp.run (proven by vincio.testing.run_signature). `.app` is the escape hatch to every deep method
