...
| **PerfBench** | compile/retrieval/run latency, cache speedups, concurrent throughput, TTFT; the single-pass feature arena — selection byte-identity, large-pool selection equivalence, bounded BM25 top-k identity, and a compile-speedup ratio floor so an erased win fails the build | cold paths / a silently-eroded optimization |
...
| **ErgonomicsBench (5.3)** | is the one-line `vincio.tasks` front door real, faithful, and uncapped? — conciseness (each of the five jobs plus the `Flow` is one entry point, benchmarked head-to-head against LCEL / the LlamaIndex query engine / DSPy / Haystack in `competitive.py`), compiles-byte-identical (the ad-hoc form lowers to the same packet and `RunResult` as the verbose builder form, proven with the shared `vincio.testing.run_signature` harness), and escape-hatch-total (`.app` reaches every deep method; nothing shadowed) | a fistful of string-keyed builder calls (the verbose `ContextApp` path) |
| **UniversalReasoningBench (7.10)** | does adaptive routing keep easy English work single-pass, use the configured model to classify languages outside a finite detector, preserve multilingual no-web intent and tool allow-lists, account the semantic route, reject freshness/tool false positives, block unsupported live claims, avoid unsound deterministic facts, activate bounded provider-neutral reasoning on non-native models, use native effort when present, correct a kernel-refuted answer within budget, and avoid recording chain-of-thought? | an ignored native-effort knob on non-reasoning models / finite language regex packs / unconditional multi-pass / unverified self-critique |
