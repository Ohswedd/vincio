...
long-context needle recall (the context governor), structured-output validity,
web freshness, and universal reasoning.
...
Universal reasoning has two dedicated Tier-L drivers. On the dated 2026-07-08
six-case run, [`reasoning_uplift_live.py`](reasoning_uplift_live.py) moved
...
routed 5/5 Spanish, Japanese, Arabic, Swahili and Chinese cases with GPT-4.1
mini on 2026-07-07.
These are reported capability samples, not general model scores; provenance and
sample sizes live in [`reference/live_snapshot.json`](reference/live_snapshot.json).
...
| **Ds4ProviderBench (7.4)** | a running `ds4-server` (self-hosted DeepSeek V4) as a first-class provider, proven offline on a recorded fixture via an injected transport — a chat response replays through the permissioned runtime to the same answer with the thinking trace excluded (`families.ds4_provider.chat_round_trips`) and an SSE stream reconstructs the same text and disk-KV usage (`…stream_round_trips`); the reasoning controller's effort/budget drives DS4's thinking switch, off for a plain call (`…thinking_drives_reasoning`); the DS4 models resolve to a priced, `self_hosted`-flagged $0 profile with the coverage gate green while the silent-$0 gate still bites a paid model at $0 (`…self_hosted_priced_zero`, `…silent_zero_gate_still_bites`); a localhost endpoint pins to `on_prem`, admitted when allowed and refused when not (`…residency_on_prem_fail_closed`); and DS4's disk-KV hits fold into the cache telemetry with the stable-prefix layout keeping a reusable prefix (`…kv_reuse_signal`) | a thin OpenAI passthrough with no thinking control, no KV accounting, and a silent $0 |
| **WebSearchBench (7.6)** | the universal browsing plane, proven offline over an injected transport and the deterministic static backend. **Token efficiency & grounding:** a reference page reduces to only the passages relevant to the model's own query at ≥10x fewer tokens (`families.web_search.extract_reduction`, ~100x on the fixture) with the known fact surviving and chrome excluded (`…relevant_recall`, `…boilerplate_excluded`), code blocks preserved verbatim (`…code_block_preserved`), and section mode returning a coherent whole section (`…section_fidelity`). **Security:** obfuscated IP-literal / wildcard-DNS SSRF bypasses refused pre-egress (`…ssrf_ip_literal_blocked`), auto-fetch firing only on genuine intent not an incidental URL (`…intent_gating_correct`), a private/loopback fetch refused (`…policy_refuses_private_hosts`). **Robustness:** walls / paywalls / JS-shells flagged unavailable (`…availability_detected`); a crawl bounded on every axis, deterministic, and trap-resistant (`…crawl_deterministic`, `…crawl_bounded`), verifiable and lossless to documents/dataset (`…crawl_verifies`, `…dataset_roundtrip`). **Universality:** a model **without** native function calling runs the identical search→read→answer loop through the text protocol a native model runs through vendor tool calls (`…universal_tool_loop`, `…protocol_lowered`); every read re-derives offline from its snapshot (`…evidence_verifies`); extraction under 50 ms (`…extract_ms`). Plus a live **uplift** track (`web_search.freshness`) measuring the same model direct vs with web search on post-cutoff facts | forwarding the fetched page (or its stripped text) into context; a hosted provider-only search tool that local models cannot call; a naive fetcher with a canonical-IP-only SSRF check and an unbounded crawl |
| **UniversalReasoningBench (7.10, extended 7.11)** | adaptive reasoning as a provider-independent mechanism: internal plan mode structures a deep multi-step request with typed dependency-ordered steps from one accounted planning call while simple work never pays for planning; a fabricated source attribution (a URL or "according to" domain in neither evidence nor request) is refuted and withheld while honest evidence-host citations are never flagged; reviewed routing cases include freshness hard negatives, user web opt-out, exact tool-match precision, unsupported live-claim refusal and deterministic-fact precision; multilingual cases cover Spanish, Japanese, Arabic and a language misidentified by the finite offline detector, gating model-native correction, multilingual no-web intent, tool allow-listing and routing-call accounting. A simple English request stays one-pass while a hard request activates bounded candidates on a non-reasoning model; native models receive adaptive effort; an offline arithmetic refutation forces correction and re-verification within the pass ceiling; receipts carry no scratch work. The `reasoning.universal` uplift entry has Static illustrative arms; `reasoning_uplift_live.py` measures answer uplift and `reasoning_multilingual_live.py` measures model-native routing across Spanish, Japanese, Arabic, Swahili and Chinese on OpenRouter | a native `reasoning_effort` field that non-reasoning models ignore; finite language-specific regex packs; unconditional multi-pass prompting; unverified self-critique |
| **RagAnchorsBench (7.7)** | context anchors keep a task frame present across a chain of calls at a flat, tiny cost, proven offline and deterministically. **Token efficiency:** a PRD/spec/brand corpus is distilled into a constraint-first brief ≥10x smaller (`families.rag_anchors.brief_reduction`, ~28x on the fixture). **Frame retention:** the frame's constraint survives into the packet on a query that never mentions it (`…frame_retained_on_mismatch`) while the identical query under pure per-query RAG loses it (`…pure_rag_loses_frame`), and a query that matches an anchor's detail retrieves it *alongside* the frame (`…tier2_detail_survives`). **Guarantee & safety:** the pinned frame is present under a tiny window, a zero-evidence task type, a footprint ceiling, and a conflicting higher-authority chunk (`…frame_guaranteed_everywhere`), always within budget — the frame is reserved off the total with a compress-then-truncate ladder so injecting it never pushes the packet over the window (`…budget_never_exceeded`); the brief is deterministic and content-hash-cached (`…brief_deterministic`) and rendered first (`…frame_first_position`). Plus a live **uplift** (`rag_anchor_uplift_live.py`) — the same model, stuff-all-files vs pure-RAG vs anchors, on a rule the coding tasks never mention | pasting every MD file into every call (token-hungry); pure per-query RAG that silently drops a globally-binding constraint on a lexical miss |
| **LagerBench (7.8)** | reasoning-driven retrieval (LAGER) measured against the **real in-repo baseline**, offline and deterministic. **Multi-hop honesty:** on a bridge corpus whose causal claim shares zero content words with the query while forty distractor documents are saturated with them, LAGER reaches the bridge by graph traversal (`families.lager.multi_hop_lager`) while the out-of-the-box hybrid top-k pipeline at the same evidence budget honestly misses it (`…beats_topk_on_multihop`). **Token efficiency:** for a query both pipelines answer correctly, the evidence pack is ≥3x smaller than the classic default context (`…token_efficiency`, ~23x on the fixture). **Laziness:** an easy query terminates in exactly one round (`…lazy_rounds_easy`), evidence volume varies with query complexity (`…no_fixed_k`), and an unanswerable query abstains within bounds with its uncovered needs named (`…impossible_abstains`). **Integrity:** every extracted object re-derives byte-for-byte from a messy fixture (`…extraction_faithful`), retrieval is byte-identical across two separate OS processes (`…deterministic_across_processes`), the gated contradiction detector holds precision ≥0.8 on labeled hard negatives while catching a true negation pair (`…contradiction_precision`, `…contradiction_recall`), tampered sources fail verification (`…verification_catches_tamper`), and every decision carries a span, source, and per-round gain trace (`…explainable`). Plus a live **uplift** (`lager_uplift_live.py`) — same model, floor / classic / LAGER arms on multi-hop questions | fixed top-k chunk retrieval that ranks by query similarity (structurally blind to zero-overlap bridges); chunk stuffing that pays 400 tokens per useful sentence; a retrieval layer with no honest "cannot answer" |
...
| **HygieneBench (6.0–6.6)** | the public surface kept consistent, every public raise typed, every best-effort failure observable, every public capability reachable, every docstring true to the code, no load-bearing `assert` stripped under `python -O`, and no dead-but-resolvable public symbol (mirroring `docs_conformance` / `registry_coverage`). **Surface (6.0):** **resolvable** (every public subpackage `__all__` name is a live attribute, with no duplicate or malformed entries — no dead surface; `families.hygiene.surface_dead_symbol_free`), **frozen** (the classified surface matches the committed `docs/reference/subpackage-surface.txt`, so any `__all__` change is a reviewed edit; `…surface_frozen`), and **gate bites** (an injected dead symbol, a duplicate, and a malformed `__all__` are each caught; `…surface_gate_detects_tamper`), folded into one headline `…surface_consistency`. **Error contract (6.1):** every error raised on a public entry point derives from `VincioError` — the `ContextApp` (`app.*` verb) surface raises no bare built-in (`…error_contract_app_verbs_clean`), the classified baseline of accepted public built-in raises matches the committed `docs/reference/error-contract.txt` (`…error_contract_frozen`), and the detector provably catches an injected public built-in raise while ignoring an encapsulated one (`…error_contract_gate_detects_tamper`), folded into one headline `…error_contract_conformant`. **Observable failure (6.2):** no public module swallows a broad exception silently — every broad `except` (or `contextlib.suppress(Exception)`) re-raises, records its failure (a logger call or `note_suppressed`), or carries a justifying `# noqa: BLE001` (`…observable_failure_clean`), and the detector provably flags an injected silent swallow while ignoring a logged one (`…observable_failure_gate_detects_tamper`), folded into one headline `…observable_failure_conformant`. **Wire-or-retire (6.3):** every formerly-unhooked public capability is reachable through a production path — each entry in a frozen ledger resolves to a live reach (an `app.*` verb, an engine method, a registration helper, or a public class member) and, for a wired one, is referenced by production code outside its defining module (`…wire_or_retire_clean`), and the detector provably bites on an unreachable reach and a wired symbol with no production caller (`…wire_or_retire_gate_detects_tamper`), folded into one headline `…wire_or_retire_conformant`. **Docstring / behaviour parity (6.4):** every docstring that advertises a behaviour either performs it or is corrected — the budget allocator exposes no `redistribute` reclaim and hands every non-fixed token to the flexible blocks (`…docstring_parity_budgeting`), the compression tuner gates on the `faithfulness` metric its docstring names while `compression_faithfulness` / `faithfulness_preserved` measure fidelity offline (`…docstring_parity_compression`), a store-less default-deny consent ledger refuses an ungranted subject and a grant flips it — the federated default-deny demonstration, deterministically (`…docstring_parity_consent`), and `MemoryEngine.delete` delegates to `forget` with the audit semantics preserved (`…docstring_parity_memory`) — each re-derived from the live code and folded into one headline `…docstring_parity_conformant`. **`-O` robustness (6.5):** no public module carries an `assert` that carries control-flow weight and would silently vanish under `python -O` — every load-bearing one is an explicit guard that raises a `VincioError`, a genuine never-happens invariant kept as an `assert` carries a justifying `# noqa: S101` (`…assert_robustness_clean`), and the detector provably flags an injected bare `assert` while ignoring a marked one (`…assert_robustness_gate_detects_tamper`), folded into one headline `…assert_robustness_conformant`. **Reachability (6.6):** no dead-but-resolvable public surface — every public symbol (top-level `vincio.__all__` plus every public subpackage `__all__`) is either *used* in the code corpus or declared, with its structural reason (`BASE` / `OPTDEP` / `WIRING`), in the frozen `docs/reference/reachability.txt` (`…reachability_clean`), the baseline is exactly the live unreferenced set (`…reachability_frozen`), and the detector provably bites on an injected dead symbol and a stale baseline entry (`…reachability_gate_detects_tamper`), folded into one headline `…reachability_conformant`; companion budgets gate the count of audited subpackages, the wired-capability count, that the silent-swallow / unmarked-assert counts are zero, and that all 1,700 public symbols are audited | a name in `__all__` that resolves to nothing reads as supported API while doing nothing; a bare `ValueError`/`KeyError` leaking off a public verb breaks `except VincioError` silently; a broad `except` that swallows its exception with no log or metric hides a real bug; a public capability with no `app.*` verb, example, or caller reads as supported API while being unreachable; a docstring that promises behaviour the code no longer performs misleads a reader who trusts it; a load-bearing `assert` vanishes under `python -O`; a public symbol used nowhere reads as supported API while being dead |
