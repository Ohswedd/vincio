...
Universal reasoning has two dedicated Tier-L drivers. On the dated 2026-07-07
small-sample run, [`reasoning_uplift_live.py`](reasoning_uplift_live.py) moved
Llama 3.2 3B from 0/4 exact direct answers to 3/4 through Vincio, with all three
stable tasks deterministically verified; the current-fact answer was safely
withheld after two web snapshots verified but the claim failed validation.
[`reasoning_multilingual_live.py`](reasoning_multilingual_live.py) correctly
routed 5/5 Spanish, Japanese, Arabic, Swahili and Chinese cases with GPT-4.1 mini.
