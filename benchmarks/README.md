...
| **WebSearchBench (7.6)** | the universal browsing plane, proven offline over an injected transport and the deterministic static backend — a reference page reduces to only the passages relevant to the model's own query at ≥10x fewer tokens than the boilerplate-stripped page (`families.web_search.extract_reduction`) with the known fact surviving the reduction and chrome excluded (`…relevant_recall`, `…boilerplate_excluded`); every read re-derives offline from its content-hashed snapshot with a tampered snapshot caught (`…evidence_verifies`); a private/loopback fetch is refused pre-egress as a typed error (`…policy_refuses_private_hosts`); and a model **without** native function calling completes the identical search→read→answer loop through the text protocol that a native model completes through vendor tool calls — same tools, same sequence, same answer, with the request provably lowered (`…universal_tool_loop`, `…protocol_lowered`) — extraction itself under 50 ms (`…extract_ms`) | forwarding the fetched page (or its stripped text) into context; a hosted provider-only search tool that local models cannot call |
