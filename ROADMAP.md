...
- **1189 tests passing offline in ~5s; ruff + mypy clean**; thirty-three runnable examples; the VincioBench `generation` / `governance` families hold the 1.9 guarantees under CI-gated budgets (172 budgets, 51 SLOs). Everything is additive behind the new `vincio.generation` subpackage, the `vincio[gen-docx|gen-pdf|gen-pptx|ocr|parquet|msg]` extras, and `@experimental` markers on the frozen 1.0 API — the milestone carries no deferred follow-ups.
