...
- **1386 tests passing offline in ~5s; ruff + mypy clean**; thirty-five runnable examples; VincioBench holds the 2.0 guarantees under CI-gated budgets (18 families, 217 budgets, 65 SLOs). The flat `app.<method>` API remains fully supported alongside the facades; the public-API contract moves to `2.0`. Known limitation: native filter *pushdown* ships server-side for Qdrant and pgvector (the structured/`jsonb` backends); the Pinecone/Weaviate/Milvus/Elasticsearch compilers ship and are unit-tested, and those adapters apply the structured filter client-side until their stored format indexes flat fields — a follow-up that needs no further schema break.
