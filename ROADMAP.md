...
hardened isolation. **2.0** is the one breaking window — the structural refactor the frozen surface
cannot make additively, plus a flagship multimodal-native Context Packet that genuinely needs it.
**2.1** and **2.2** are additive again — distributed execution, executed fine-tuning, a served (still
self-hosted) observability plane, and an environment-based agentic eval harness. **3.0** is the next
breaking culmination: a single declarative self-improvement contract, provable erasure with consent
modeling, and an async-first canonical core.
...
### 🚧 1.8 — Provider/model rotation & swap regression: the migration safety net (vs LiteLLM Router, Braintrust/LangSmith model experiments)
...
- *Definition of done:* capability preflight, cost/latency routing, the `SwapGate`'s pass/fail diff, model-swap regression with flake quarantine, shadow dual-dispatch, canary auto-rollback, and the lifecycle migration proposal all covered offline against the deterministic mock and recorded provider cassettes; gated by the VincioBench **reliability** (capability-guard correctness, lifecycle-error classification), **cost** (routing cost/latency trade, Google/Vertex batch parity), **evals** (swap-gate significance, replay-diff fidelity), and **scale** (canary rollback under load) families, with the SLOs extended and a new runnable example (`31_swap_regression.py`) that swaps a model end to end through the gate and a canary.

### 🚧 1.9 — Documents & images flow OUT — cited, governed, eval-gated artifacts (vs Unstructured/python-docx writers, gpt-image-1, OpenAI/Gemini TTS, C2PA)
...
- 🚧 **Document generation engine — cited DOCX/PDF/PPTX/HTML/Markdown** — a new `vincio.generation` subpackage whose `DocumentBuilder` turns a validated `OutputContract` result (or a Context Packet) into rendered artifacts: Markdown/HTML dependency-free, DOCX via python-docx, PDF via WeasyPrint/reportlab, PPTX via python-pptx behind extras (`vincio[gen-docx|gen-pdf|gen-pptx]`). Because it consumes a *validated* result, the document is grounded by construction; it supports **structural document contracts** (`DocumentContract`: required sections, table column specs, length bounds, citation-per-section) with formatting-only repair mirroring the JSON-repair path, and records every render in the hash-chained audit log as a `document_generate` event carrying the source evidence ids. Adds template/form filling (typed citation-aware slots, DOCX merge fields, PDF AcroForm) and a redline generator pairing the existing `DOCUMENT_COMPARISON` intent with tracked-change DOCX output.
- 🚧 **Cited-report assembly with resolved citations & bibliography** — a `CitedReportBuilder` takes a validated answer plus its `EvidenceItem`s and renders inline `[E1]`-style markers resolved to footnotes/endnotes, a generated source table/bibliography, and per-claim provenance (trust level, `source_uri`, page/section), output to any `DocumentBuilder` format. Adds **field/claim-level citation contracts** (the schema annotation "this field must cite"), sentence-level citation-coverage metrics, and an optional NLI/entailment check (`evals.metrics` groundedness) that the cited evidence actually *supports* the claim — replacing today's flat "one valid citation anywhere" membership check in `output/validators.py`. Citation extraction reuses `output/parsers.extract_citations`, so the resolution apparatus reads the same markers the validator already trusts.
- 🚧 **Image generation/editing provider abstraction with provenance** — adds `generate_image` / `edit_image` / `variation` as a new capability surface over OpenAI `gpt-image-1`, Gemini/Imagen, and an HTTP/Replicate adapter, with a neutral `ImageGenRequest` (prompt, size, n, quality, mask, reference images, seed) and `ImageGenResponse` (image refs, `revised_prompt`, usage, cost), plus a `MockImageProvider` for offline tests. Every generated asset auto-attaches a media-aware C2PA manifest, is metered against the budget, and runs through the guardrail/eval path. `governance.transparency.mark_synthetic_content` becomes **media-aware** (accepts bytes, embeds credentials into file metadata, adds a `digitalSourceType` and an invisible-watermark hook) so the marker can finally bind images/audio, not just `str` — registry capability flags advertise which providers can generate or edit.
- 🚧 **TTS / speech-synthesis output modality** — a `synthesize_speech` surface with a neutral voice/format/speed model over OpenAI TTS, Gemini TTS, and ElevenLabs/Cartesia plus a deterministic mock; outputs are marked with audio provenance and metered against budgets, unified with the realtime audio path so synthetic speech is governed exactly like every other output. Wires `ContentPart.audio` through the OpenAI/Gemini chat providers (`input_audio` parts) so the already-typed `AudioRef` is usable as **input** outside the realtime WebSocket path, gated by registry capability flags — finally activating an input type the type system already exposes but never accepted on the chat path.
- 🚧 **OCR auto-fallback, audio transcript ingestion & richer inputs** — wires OCR into the loader: `load_pdf` detects low text-yield pages and routes them through a supplied `OCREngine` (rasterize via pypdfium2 → Tesseract/`VisionModelOCR`), recording `extractor='ocr'` per page so provenance stays honest. Adds `load_media(path, transcriber=...)` producing a timestamped, optionally speaker-diarized transcript `Document` via a `Transcriber` protocol mirroring `OCREngine` (Whisper/Deepgram/provider-audio backends), turning the dead "audio" file classification into a real ingestion path. Crops `LayoutFigure` regions and feeds them through `ImageAnalyzer`/OCR so PDF charts and diagrams become grounded, citable evidence with bounding boxes; structures JSON/JSONL/YAML into sections/tables instead of opaque text; and adds a real-parser HTML path with table extraction (`documents/parsers.py`).
- 🚧 **New input formats + forms/KYC structured extraction** — loaders (and extras) for the formats the classifier already advertises but cannot load — PPTX, EPUB, RTF, ODT, Parquet/Arrow (as `TableData`), mbox/`.msg` threads — each yielding sections/tables consistent with existing `TableData`, plus a unified **parser registry** (`documents/registry`) that replaces the if/elif suffix chain so formats register additively. Adds a forms-extraction path (Textract / Azure Document Intelligence / Google Document AI adapters behind a `DocumentAI` protocol, plus an offline heuristic key-value extractor) returning structured fields with bbox + confidence as citable evidence for the dominant invoice/receipt/ID document-AI use-case — and unifies the classifier's promises with the loader's reality so routing never lies about ingestability.
- 🚧 **EU AI Act conformity pack — Annex IV docs as generated, cited artifacts** — the dominant 2026 compliance driver, built as the document-generation engine's first governance application (`governance/transparency.py` already cites the 2 Aug 2026 transparency duty). A `RiskTierClassifier` places a configured `ContextApp` into the Act's risk tiers (prohibited / high-risk / limited / minimal) from its declared purpose, data sources, and human-oversight controls; an `AnnexIVBuilder` renders the **Annex IV technical documentation** as a cited DOCX/PDF through the 1.9 `DocumentBuilder`, drawing every field from the live config, the model/system cards, the compliance matrix, and the eval/red-team evidence Vincio already holds (so it is grounded by construction, not hand-written); an Article 27 **FRIA** (fundamental-rights impact assessment) generator and an **ISO/IEC 42001** control catalog join the `ComplianceMapper` (`governance/frameworks.py`) family. Deadline-agnostic and pluggable like the existing card formats; the conformity pack is a *view* over the running system, regenerated on every config change, recorded as a `conformity_doc` audit event.
...
- *Definition of done:* document generation in every format, structural document-contract validation, cited-report citation resolution + per-claim entailment, image-gen/TTS with media C2PA provenance, OCR auto-fallback, audio transcript ingestion, figure-to-evidence, and the new format loaders all covered offline against mocks/cassettes; gated by the VincioBench **output**, **governance**, **rag**, and **security** families (document-contract validity and citation coverage in `output`; media-provenance binding and disclosure in `governance`; OCR/transcript/figure evidence recall in `rag`; generated-media safety in `security`), with new examples and SLOs for each. Depends on 1.7.

### 🚧 1.10 — The loop closes itself: continual, online, safe self-improvement & the agentic frontier (vs DSPy 3 SIMBA, OpenAI/Gemini Deep Research, Letta/MemGPT, Anthropic computer-use)
...
- *Definition of done:* gated by the VincioBench `loop`, `agentic_evals`, `agent`, `security`, and `memory` families — drift-triggered gated re-optimization/rollback correctness and held-out non-regression in `loop`; reflector diagnosis lift and deep-research citation/grounding/quality in `agentic_evals`; level-parallel DAG and `plan_and_execute` replanning in `agent`; `IsolationBackend` enforcement and hosted-tool permissioning in `security`; memory-as-tools provenance and in-loop compaction in `memory` — all covered offline against mocks and cassettes, with new runnable examples and new SLOs, and persisted online state proven restart-safe and worker-aggregatable.

### 🚧 2.0 — The one breaking window: the structural refactor, async-first stores, the multimodal-native Context Packet & enterprise endpoints (vs LiteLLM/Bedrock/Vertex, the OTel GenAI agentic conventions)
...
- *Definition of done:* the intent is that facade decomposition with deprecated-alias bridges, async-store throughput under concurrency, the typed event catalog + OTel agentic conventions, multimodal-native packet selection/budgeting/citation, `FilterSpec` native pushdown with tenant-scope correctness, the three enterprise endpoints, always-on egress DLP, and the signed audit chain are all covered offline — gated by the VincioBench `scale`, `rag`, `governance`, `reliability`, and `perf` families (cold-start + async throughput in `scale`/`perf`; multimodal selection and filter-pushdown correctness in `rag`; egress DLP, signed-chain verification, and enterprise-endpoint residency in `governance`; capability-guarded enterprise failover in `reliability`) — with a new runnable example, the SLOs, and the mechanical deprecation runway (deprecated-in / removed-in versions for every collapsed alias) documented before the window closes.

### 🚧 2.1 — Scale out & train for real — distributed execution, executed fine-tuning, served observability (vs Temporal/Ray, DSPy BootstrapFinetune, Langfuse/Phoenix)
...
- **Distributed durable-execution backend** — a `RuntimeBackend` (`agents/backends.py`) that runs the same `StateGraph` / `Workflow` / `Crew` on Temporal or Ray with queued steps, worker pools, and cross-restart durability, extending the no-lock-in backend story from export-only to distributed run. Concurrent resumers are made safe by optimistic-concurrency on the checkpointer (checkpoint-version CAS) plus a TTL `running` lease on each graph thread, so two workers can't double-execute a step; `agents/graph.py` and `workflows/engine.py` gain true BSP parallel super-steps and level-parallel sub-agent spawning (`Send` / map-reduce) for fan-out workloads. Single-process asyncio remains the default backend and the lease/CAS metadata rides the same checkpoint records, so a run can move between backends without losing its evidence ledger or trace.
- **Executed distillation & provider fine-tune jobs** — the `StudentTrainer` in `optimize/distill.py` graduates from an injected no-op into shipped trainers that submit and poll OpenAI / Anthropic / Gemini fine-tune jobs, register the resulting model in `providers/registry.py`, and gate-promote the student into the routing cascade through the existing swap gate — so the flywheel produces an actual cheaper model, not just grounded JSONL. The export gains semantic dedup and a truncation guard so the training set stays diverse and faithful to the captured traces. Offline, the job lifecycle runs against mocked provider endpoints (cassette-backed submit/poll/status) and the promotion decision is fully deterministic.
- **Served (self-hosted) observability & alerting plane** — a lightweight optional local server (`observability/viewer.py`) over an indexed SQLite trace/cost store with time-bucketed pre-aggregates, retention, and rollups that replace the O(n) JSONL scans: live trace tail, attribute/tenant/model search, latency and cost p50/p95/p99 charts, session timelines, and cost-by-dimension dashboards. An `AlertSink` protocol (webhook / Slack / PagerDuty / Prometheus, `observability/exporters.py`) plus a rule engine (threshold, EWMA/Welford anomaly, SRE burn-rate, `observability/finops.py`) runs over the same cost ledger and event bus, with tail-based error-prioritized sampling so failures are kept and noise is dropped. The zero-dependency static viewer stays; this plane is opt-in, runs on your infrastructure, and emits on the same audit chain — never a hosted service.
- **Redis-backed shared server state + content-capture controls** — response, tool, and idempotency caches and rate limiting move onto Redis (`storage/redis.py`) so multi-worker uvicorn deployments stay coherent, with per-tenant quotas, and a first-class `vincio serve` launcher (`cli/main.py`) closes the audit finding that server mode shipped but was unlaunchable, adding health/readiness/metrics endpoints and graceful shutdown. Prompt/completion content capture is gated behind an opt-in flag with truncation and PII-redaction applied at the export boundary (`tools/runtime.py`, `observability/otel.py`) before any content reaches OTel events, JSONL, or the viewer — so the served plane never widens your data-exposure surface.
- **Quantization + two-stage retrieval & batteries-included local neural models** — binary/scalar quantization and Matryoshka two-stage retrieval (coarse search on truncated/quantized vectors, rerank on full precision) surface in the Qdrant and pgvector adapters (`storage/qdrant.py`, `storage/postgres.py`), reusing the shipped `mrl_truncate` and PLAID building blocks. Optional-dependency real local models arrive batteries-included: an ONNX/fastembed dense embedder, a local cross-encoder reranker, a real SPLADE encoder, and a ColBERT token-embedder (`retrieval/embeddings.py`, `retrieval/rerankers.py`), plus a native llama.cpp/GGUF in-process provider with on-device embedding (`providers/local.py`) — so air-gapped and edge deployments get semantic quality and true offline inference with no server, behind the same `Embedder` / `Reranker` / `Index` / provider interfaces.
- *Interconnection:* scale rides the 2.0 async contracts and facades rather than forking them — the distributed backend runs the same `StateGraph` / `Workflow` on the same checkpointer; the trained student promotes through the same significance swap gate as a model rotation; the served plane reads the same indexed trace/cost store and emits on the same event bus; and quantized two-stage retrieval reuses the same `Index` protocol and embedder/reranker interfaces, so distribution and serving are views over the same organs, not a parallel system.
- *Edge over specialists:* the field sells a durable-execution cloud (Temporal/Ray, LangGraph Platform), a fine-tune toolkit (DSPy BootstrapFinetune), and an observability SaaS (Langfuse/LangSmith/Phoenix) as three separate hosted products. Vincio keeps the single-process path the default and the distributed path lock-free, gates the trained student on the same replay-plus-significance swap gate so the flywheel ships a cheaper model only if it provably doesn't regress, and serves a dashboard people actually watch inside the same audit chain and cost ledger — distributed run, executed gated fine-tuning, and served observability all on your infrastructure, in one trace, in-process not a control plane.
- *Definition of done:* the intent is that distributed execution with lease/CAS and BSP fan-out, executed-and-gated fine-tuning, the served observability and alert plane over the indexed store, Redis shared state with `vincio serve`, and quantized two-stage retrieval plus the local neural models are all exercised offline against mocks and cassettes, and gated by the VincioBench `scale`, `loop`, `perf`/`rag`, and `cost` families — distributed durability and multi-worker cache coherence in `scale`, the executed-distillation swap-gate in `loop`, indexed-store query latency and quantized-retrieval recall in `perf`/`rag`, and burn-rate alerting in `cost` — landing with new runnable examples and SLOs for each.
