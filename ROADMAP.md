...
- **1497 tests passing offline in ~6s; ruff + mypy clean**; thirty-six runnable examples; VincioBench holds the 2.1 guarantees under CI-gated budgets (231 budgets, 71 SLOs): distributed durability + multi-worker shared-state coherence in `scale`, the executed-distillation swap-gate in `loop`, quantized two-stage recall in `rag`, and burn-rate/EWMA alerting in `cost`. Everything is additive behind the `vincio.agents` distributed entry points, `vincio.providers.finetune`, `vincio.observability` (store/viewer/alerts), `vincio.storage.shared_state`, and `vincio.retrieval.quantization` plus the local-model classes, all `@experimental` on the frozen 2.0 surface. The 2.1.1 follow-up closes the three review-surfaced limitations at their root: channel-default reducers (`StateGraph(defaults=…)`) retire the map-reduce seed-node workaround; `assert_backend_conformance` makes the Ray/Temporal export adapters prove they reproduce the native engine; and the local neural models (SPLADE/cross-encoder/fastembed) accept an injected model so their real forward/predict/embed paths run offline against faithful fakes (no `# pragma: no cover`). **The 2.1 milestone carries no deferred items.**

See the [CHANGELOG](CHANGELOG.md) for the complete 2.1.0 and 2.1.1 notes.

### ✅ 2.2 — Prove it on the world's benchmarks: environment eval, agentic leaderboards & the agent fabric (vs τ-bench/SWE-bench/WebArena, AGNTCY, generative UI) (shipped)

*The field now judges agents by task success in stateful environments and on public leaderboards, lets them discover one another through agent directories, and streams structured UI to frontends. With distributed execution and computer-use already shipped at 2.0/2.1, this milestone makes Vincio **measurable** on the benchmarks buyers actually compare on, **composable** into a governed agent fabric, and **embeddable** in interactive products — turning leaderboard numbers into a closed-loop training signal rather than a post-hoc score. Entirely additive behind `@experimental(since="2.2")` entry points on the 2.0/2.1 surface, all self-hosted; the benchmark adapters and registry clients use only the core `httpx` dependency, and the reference environments run deterministically in-process.*

- ✅ **Stateful-environment eval harness + agentic benchmark adapters** — an `Environment` protocol (`reset` / `step` / `observe` / `verify`) in `evals/environment.py` with deterministic in-process reference environments (`ToolEnvironment` + `make_retail_environment`, a τ-bench-style retail world) and a **task-success oracle** (declarative `StateCheck`s over the world's end state), so the `EnvironmentSimulator` drives an agent *policy* through a *mutable* world and projects it onto the existing `Trajectory` (`evals/trajectory.py`) — scoring **verifiable end-state**, not turn-by-turn plausibility. The five adapters ship in `evals/benchmarks.py` behind one `BenchmarkAdapter` contract — **SWE-bench Verified** (fail-to-pass/pass-to-pass transition), **τ-bench/τ²-bench** (database end state via the environment oracle), **GAIA** (normalized exact match), **WebArena** (functional check), and **BFCL** (AST match) — each pinning its task set by content hash for reproducibility and degrading to recorded-fixture replay offline (fixtures in `benchmarks/fixtures/`), exercised by the VincioBench runner and projecting to an `EvalReport` the Pareto optimizer can consume.
- ✅ **Retrieval evaluation harness (recall@k / nDCG / MRR / context-precision)** — a golden-set harness scoped to retrieval (`evals/retrieval_eval.py`: `RetrievalEvaluator` / `RetrievalGoldenSet` / `RetrievalConfig`) that quantitatively benchmarks embedder / reranker / chunker / index configs by reusing the retrieval metrics, with versioned **index regression artifacts** keyed on `(embedder, chunker, corpus hash)` in `storage/index_regression.py` (`IndexRegressionStore` over the `MetadataStore`) so a re-embed or a chunking tweak that regresses recall is caught against a stable golden query set. Results render through `EvalReport` and `retrieval_regression(...)` gates on recall/nDCG deltas using the *same* significance machinery (`ab_test`) as a model swap, making the offline-vs-real retrieval tradeoffs a first-class CI gate instead of a vibe check.
- ✅ **Agent registry / discovery (AGNTCY / ACP)** — an `AgentDirectory` (`registry/directory.py`) and capability-discovery layer over the existing A2A **Agent Card** (`a2a/protocol.py`) with an **allow-list governance gate** (`AllowListGate` in `security/access.py`), so orgs control which agents and servers are reachable. An **AGNTCY / ACP** (REST-native Agent Connect Protocol) adapter (`registry/acp.py`: `ACPClient` + Agent-Card ↔ ACP-manifest mapping) makes Vincio span both interop camps, and an **MCP Registry** discovery client (`registry/mcp_registry.py`) resolves MCP servers from the official registry under that same allow-list. Point-to-point delegation becomes a *governed, discoverable fabric* — every resolution recorded as an access decision on the audit chain (`app.agent_directory(...)`).
- ✅ **Generative UI / AG-UI streaming protocol** — an **AG-UI / MCP-UI** compatible streaming UI-event protocol (`server/agui.py`: `AGUIEvent` + `run_stream_to_agui` / `agent_stream_to_agui`) and the SSE endpoint `POST /v1/apps/{app_id}/agui` in `server/app.py`, so runs drive interactive frontends with structured UI deltas over the existing `astream` path, plus **token- and tool-event streaming** from `agents/executor.py` (`AgentExecutor.astream`) and `agents/crew.py` (`Crew.astream`, forwarding each member's tool/text events) — matching the streaming surface `graph` and `compose` already expose — and UI resources (`MCPUIResource`) from `mcp/server.py`. The interactive frontend inherits the run's provenance, budget metering, and audit — one streamed run, not a bolt-on UI layer.
- *Interconnection:* benchmarks and the fabric ride the organs that already exist — the environment harness reuses the `Trajectory` model and feeds the **same optimizer / Pareto loop** that tunes prompts, routing, and budgets; the benchmark adapters run inside the VincioBench runner and project to `EvalReport`; the retrieval harness reuses `evaluate_gates` and `ab_test`; the agent directory extends the A2A Agent Card and the `AllowListGate`; and generative-UI events stream over the existing `astream` path and land on **one trace** with the same cost model.
...
- **1560 tests passing offline in ~7s; ruff + mypy clean**; thirty-seven runnable examples; VincioBench holds the 2.2 guarantees under CI-gated budgets (255 budgets, 77 SLOs): environment task-success + benchmark-adapter determinism in `agentic_evals`, retrieval-eval recall/nDCG + index-version regression in `rag`, AGNTCY/ACP + MCP-registry discovery under the allow-list in `protocols`, and token/tool-event + AG-UI streaming in `agent`. Everything is additive behind the `vincio.evals` environment/benchmark/retrieval-eval entry points, `vincio.registry`, `vincio.security.AllowListGate`, `vincio.server.agui`, `vincio.storage.index_regression`, and the `AgentExecutor`/`Crew` `astream` surface — all `@experimental(since="2.2")` on the frozen 2.0 surface. **The 2.2 milestone carries no deferred items.**
