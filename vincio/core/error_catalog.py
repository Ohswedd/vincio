...
ERROR_CATALOG: dict[str, ErrorCatalogEntry] = {
    ...
    for e in (
        ...
        _entry(
            ...
            "automatic retry; persistent timeouts indicate provider degradation, so "
        ...
        _entry(
            ...
            "A chart could not be built from the result. Confirm the result has rows "
            "and that any `x`/`y`/`color` you name are result columns; install "
        ...
        _entry(
            ...
            "Tool call breached its contract",
            "A tool declared pre/post-conditions and the actual call broke one: an "
            "argument failed a `requires` clause, or the result failed an `ensures` "
            "clause. Fix the arguments to meet the precondition, or treat a "
            "post-condition breach as a bug in the tool; the runtime refuses an "
            "out-of-contract result rather than returning it.",
        ),
        # --- agents ---
        _entry(
            "AGENT_ERROR",
        ...
        _entry(
            ...
            "Agent identity / delegation / credential failed verification",
            "A DID, identity document, delegation chain, or credential did not "
            "verify from the bytes. Check `.details` for the failing artifact: a "
            "sub-delegation may amplify its parent's grant (only attenuation is "
            "allowed), a signature may not bind to its issuer DID, or a key may "
            "have been rotated/revoked. Re-issue the artifact with `app.identity(...)`.",
        ),
        # --- governance ---
        _entry(
            "GOVERNANCE_ERROR",
        ...
        _entry(
            ...
            "Answer certificate did not check",
            "A deterministic verifier refuted the answer (an arithmetic, unit, "
            "temporal, schema, constraint, or citation check failed) and "
            "`app.verify_reasoning(..., raise_on_refute=True)` was asked to raise. "
            "Inspect `VerifiedAnswer.certificate.refutations`, fix the refuted "
            "claim, or run with self-correction so the orchestrator repairs it.",
        ),
        _entry(
            "BEHAVIOR_VIOLATION",
            "Agent trajectory violated a behavior spec",
            "A `RuntimeMonitor` / `Shield` found a `BehaviorSpec` property breached: "
            "a forbidden action, a missing precondition (e.g. a write before "
            "approval, a claim before retrieval), or a violated invariant. Use a "
            "shield in `block`/`repair` mode to prevent the action, or correct the "
            "plan so the property holds.",
        ),
        _entry(
            "PROGRAM_SYNTHESIS_FAILED",
            "Synthesized program failed verification",
            "A `synthesize(...)` program failed to run on its examples or a declared "
            "property (schema, row-count, field-range) was refuted. Inspect "
            "`SynthesizedProgram.certificate.refutations`, fix the op pipeline or "
            "the property, and re-synthesize; a refuted program is never run.",
        ),
        ...
        _entry(
            ...
            "Declare exactly one of `participant=` (static) or `capability=` "
            "(discovered) per `Saga` step; register a participant binding for every "
            "org a static step names; for a discovered step pass a governed "
            "`directory=` / `binder=` so the capability resolves to an allowed, "
            "reachable candidate at dispatch time. Give the saga at least one "
            "uniquely-named step and pass a `saga_id` that exists in the durable "
            "store when calling `resume`. A saga whose forward step fails does not "
            "raise; it compensates and returns a SagaResult with "
            "`status='compensated'`.",
        ),
        _entry(
            "COMPENSATION_FAILED",
        ...
        _entry(
            "ASSURANCE_ERROR",
            "Assurance case or certification could not proceed",
            "Give every leaf `Claim` at least one `Evidence` item (or list the "
            "kinds it demands on `required_evidence`), bind each `Evidence` to an "
            "artifact that exposes a verifiable verdict (an eval gate, a "
            "`GovernanceVerifier` report, a reasoning `Certificate`, an audit log, "
            "an identity/delegation chain, or an AI-BOM), and reference an "
            "`Incident` only to a claim that exists in the case. A claim whose "
            "evidence is missing, stale, or falsified does not raise; it is"
            "pinpointed on the `AssuranceReport` and the case `holds` is False; "
            "re-verify an `AssuranceCase` or `CertificationReport` with its own "
            "`verify()`.",
        ),
    )
}
