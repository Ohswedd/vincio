...
# Media produced algorithmically *from data* — a chart, a rendered table, a data
# export. Distinct from AI-generated media: it is a faithful, deterministic
# rendering of real values, not synthetic content (``is_synthetic=False``).
_DATA_DRIVEN_MEDIA = "http://cv.iptc.org/newscodes/digitalsourcetype/dataDrivenMedia"
...
def mark_data_driven_content(
    ...
    *,
    ...
    """Build a provenance manifest marking ``content`` as **data-driven** media.

    The content-credential analogue of :func:`mark_synthetic_content` for a chart,
    a rendered table, or a data export: media produced algorithmically *from data*
    rather than generated by a model. It binds the bytes (or text) by SHA-256 the
    same way, but carries the IPTC ``dataDrivenMedia`` digital-source-type and
    ``is_synthetic=False`` — a faithful rendering of real values is not synthetic
    content, and saying so honestly is the point of the credential. Pass a
    ``signer`` to attach a cryptographic signature over the binding payload (verify
    it later with :func:`verify_manifest`)."""
    import vincio

    manifest = ProvenanceManifest(
        claim_generator=f"vincio/{vincio.__version__}",
        is_synthetic=False,
        digital_source_type=_DATA_DRIVEN_MEDIA,
        model_id=model_id,
        provider=provider,
