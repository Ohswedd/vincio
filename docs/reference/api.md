...
| `swap_regression(dataset, *, candidate_model, baseline_model=, metrics=, quality_metric="semantic_similarity", alpha=0.05, repeats=1, flake_quarantine=True)` / `aswap_regression` *(experimental, 1.8)* | swap only the model on a fixed dataset → `SwapRegressionReport` (significance, cost/latency, worst slices) |
