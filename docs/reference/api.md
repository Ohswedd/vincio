...
| `enable_training_capture(enabled=True)` *(experimental, 1.4)* | record full output + cited evidence on each trace for faithful distillation → `ContextApp` |
| `export_training_set(*, name=, traces=, limit=500, min_feedback_score=, require_grounding=True, min_support=0.5, max_examples=, path=, format="openai"\|"anthropic")` *(experimental, 1.4)* | curate grounded production traces into fine-tuning JSONL → `TrainingSet` |
